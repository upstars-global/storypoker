...
- **Components:** `reka-ui@2.9.8` (headless Radix-Vue port) — `TooltipProvider` в `App.vue`, dialog/popover/switch-примітиви в модалках, `CardsArea`, `AppHeader`, `PlayerRow`, `Timer`, `[slug].vue`. shadcn-vue ініціалізовано (`components.json`: style `reka-nova`, base `reka`, baseColor `neutral`, iconLibrary `lucide`, aliases `~/components`, `~/components/ui`, `~/lib/utils`); `cn()` у `app/lib/utils.ts` (clsx + tailwind-merge); deps `class-variance-authority`, `tw-animate-css`, `@lucide/vue`. Додавати: `npx shadcn-vue@latest add <comp>` → `app/components/ui/`
  - ⚠ узгодження токенів у роботі: shadcn пише темні значення у `.dark`, а застосунок перемикає тему через `[data-theme='dark']`; shadcn `@theme inline` перевизначає `--color-primary`/`--color-muted`, що конфліктує з MUI-утилітами `text-primary`/`text-muted`
