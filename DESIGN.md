...
**Tooltip на точках** (додано 2026-07-12): наведення на будь-яку крапку (DEV чи QA) показує серію, значення (з `%`) й дату у стилізованому SVG-блоці; кожна точка має прозоре коло-приціл (`r="8"`) навколо видимої (`r="3"`) для зручнішого наведення, видима крапка - `pointer-events: none`, щоб події завжди йшли на приціл.

**Ось X** - точки розподілені рівномірно за порядковим номером раунду (ordinal, `indexToX`), а не пропорційно реальному часу, щоб близькі в часі раунди не накладались; підписи знизу - номер ISO-тижня (`w12`, `isoWeek()`), а не дата. Лінія графіка - згладжена крива (Catmull-Rom → Bezier, `smoothPath()`), а не ламана.

**Round snapshot** (додано 2026-07-25): у tooltip - клікабельний підпис `trends.viewDetails`, що відкриває `RoundSnapshotModal.vue` (вкладена `<dialog>` поверх поточної модалки) зі списком `{ім'я, голос}` усіх гравців того раунду (`round_history.votes`, повний знімок, не лише агреговані `counts`). Поруч з іменем - `RoleBadge` з тегом ролі (`roleTagForShields(shieldsMap.get(player_id))`, той самий `shieldsMap` з **поточних** shields, що й для DEV/QA split вище - те саме застереження про неактуальність ролі на момент голосування).

**Pin tooltip кліком** - hover показує превʼю (200мс `hideTooltipTimer` при виході з крапки/тултипу), клік на крапку "пришпилює" tooltip (`pinned: true`) - він більше не ховається на `mouseleave`, дозволяючи спокійно навести на кнопку "Деталі" (актуально для крайніх точок графіка, де tooltip зсунутий вбік через x-clamp). Клік на ту саму запінену крапку або на "×" у правому верхньому куті знімає пін. Tooltip позиціонується ближче до самої крапки (offset 42px замість попередніх 52px). Лейбл серії - `seriesLabel()`: QA лишається `QA`, DEV-когорта (не-QA гравці) підписана `DEV/FE/BE`, щоб не читалось як "тільки роль DEV" (колір ліній не змінювався: жовта `#ffa726` - QA, бірюзова `#26a69a` - DEV/FE/BE).
